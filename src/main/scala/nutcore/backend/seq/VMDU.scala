...
class VMDU_Embedded(val Lane_number: Int) extends NutCoreModule with HasVectorParameter {
    ...
    // currently we try to implement vlmul but will not ensure that it is working properly
    // Lane number will affect mask generation
    val state = RegInit(0.U)
    val sNone :: sWorking :: Nil = Enum(2)
    
    // we use vs1 and vs2 port to read necessary registers, vs3 is vd
    ...
    val mdu = Module(new Cluster)
    val mdu_src1 = Wire(UInt(XLEN.W))
    val mdu_src2 = Wire(UInt(XLEN.W))
    val mdu_src3 = Wire(UInt(XLEN.W))
    val mdu_func = Wire(UInt(VXUOpType.opWidth.W))
    
    // vs2 is always presented
    mdu_src1 := io.vreg.vsrc2 // io.vreg.vs2((Lane_number + 1) * XLEN - 1,  (Lane_number) * XLEN)

    
    when (VXUOpType.isSrc1Vector(io.in.bits.func)) {
        mdu_src2 := io.vreg.vsrc1 // io.vreg.vs1((Lane_number + 1) * XLEN - 1,  (Lane_number) * XLEN)
    } .otherwise {
        // rsrc2 has been processed by ISU: either from scalar reg or from imm
        mdu_src2 := gen_scalar_src(io.in.bits.rsrc1)
    }
    
    mdu_src3 :=  io.vreg.vsrc3
    
    
    mdu_func := io.in.bits.func
    
    val (io_in_ready, mdu_out_bits, io_out_valid) = mdu.access(io.in.valid, mdu_src1, mdu_src2, mdu_src3, mdu_func, io.cfg.vsew, io.out.ready, wmask)
    io.in.ready := io_in_ready && state === sNone
    io.out.valid := io_out_valid
